  ...
  if (e.target.value != "") {
    ...
    if (voucherObj.getCode() != "" && voucherObj.getStatus() == "active") {
      ...
      document.querySelector("#all-discount").textContent =
        `-${getStringFormat((billObj.getAllDiscount() + voucherUsed).toFixed(2))}`;
      document.querySelector("#final-total").textContent = getStringFormat(
...
function removeVoucher(billObj, subTotal) {
  ...
  document.querySelector("#all-discount").textContent =
    `-${getStringFormat(billObj.getAllDiscount())}`;
  document.querySelector("#final-total").textContent =
    getStringFormat(subTotal);
  document.querySelector(".toggle").onclick = (e) =>
...
function handleSplitAmountChanged(e, billObj) {
  ...
  if (e.target.value != "") {
    if (
      getStringFormat(e.target.value) != false &&
      parseFloat(e.target.value) >= 0 &&
      parseFloat(e.target.value) <= finalTotal
    ) {
      if (e.target.id == "card") {
        document.querySelector("#cash").value = getStringFormat(
          (finalTotal - parseFloat(e.target.value)).toFixed(2),
        );
      } else if (e.target.id == "cash") {
        document.querySelector("#card").value = getStringFormat(
          (finalTotal - parseFloat(e.target.value)).toFixed(2),
        );
      }
      setPaymentAmountToObj(billObj, finalTotal);
