...
import { getVoucherList } from "../api/voucher-api.js";
import { formatDate } from "../functions/shared.js";
import { closeDialog } from "./dialog.js";
import { checkDollarStringFormat } from "../utils/currency.js";
...
export function checkout(bill) {
  ...
  let voucherObj = new Voucher(
    vouchers.find(
      (voucher) =>
        voucher.code.toLowerCase() == billObj.getVoucherCode().toLowerCase(),
    ),
  );
  ...
  let finalTotal =
    voucherObj.getCode() == ""
      ? subTotalOfBill
      : voucherPrice <= subTotalOfBill
        ? subTotalOfBill - voucherPrice
        : 0;
  ...
  content.innerHTML = getCheckOutContent(
    billObj,
    services,
    voucherObj,
    floatFinalTotal,
  ...
  document.querySelector("#voucher").onchange = (e) => {
    handleVoucherInputChanged(e, billObj, vouchers, services);
  };
  document.querySelector("#voucher-delete").onclick = () =>
    removeVoucher(billObj, subTotalOfBill);
  document.querySelector(".toggle").onclick = (e) =>
    handledToggleValueChanged(e, billObj, floatFinalTotal);
  document.querySelector("#card").onchange = (e) =>
    handleSplitAmountChanged(e, billObj, floatFinalTotal);
  document.querySelector("#cash").onchange = (e) =>
    handleSplitAmountChanged(e, billObj, floatFinalTotal);
...
function getCheckOutContent(billObj, services, voucher, finalTotal) {
  let voucherUsed =
    voucher.getPrice() != ""
      ? voucher.getPrice() <= billObj.getTotal(services)
        ? voucher.getPrice()
        : billObj.getTotal(services)
      : 0;
  return `
        ...
        <hr>
            <div class="list-item" style="justify-content: space-between">
                ...
                <span>${checkDollarStringFormat(parseFloat(billObj.getTotal(services)))}</span>
            </div>
            <div class="list-item" style="justify-content: space-between">
            ...
            <div class="list-item" style="justify-content: space-between">
                ...
                <span style="color: #C97C82" id="all-discount">-${checkDollarStringFormat(parseFloat(billObj.getAllDiscount() + voucherUsed))}</span>
            </div>
            <div class="list-item" style="justify-content: space-between">
                <span><b>Total:</b></span>
                <span id="final-total">${checkDollarStringFormat(finalTotal)}</span>
            </div>
        <hr>
...
function handleVoucherInputChanged(e, billObj, vouchers, services) {
  ...
  if (e.target.value != "") {
    const voucherObj = new Voucher(
      vouchers.find(
        (voucher) => voucher.code.toLowerCase() == e.target.value.toLowerCase(),
      ),
    );
    let voucherPrice = voucherObj.getPrice();
    if (voucherObj.getCode() != "" && voucherObj.getStatus() == "active") {
      ...
      document.querySelector("#all-discount").textContent =
        `-${checkDollarStringFormat((billObj.getAllDiscount() + voucherUsed).toFixed(2))}`;
      document.querySelector("#final-total").textContent =
        checkDollarStringFormat(finalTotal.toFixed(2));
      document.querySelector(".toggle").onclick = (e) =>
        handledToggleValueChanged(
          e,
          billObj,
      ...
      document.querySelector("#error").innerHTML =
        '<i class="fa-solid fa-circle-exclamation"></i> Unavailable voucher code';
    }
  } else {
    removeVoucher(billObj, subTotal);
  }
...
function removeVoucher(billObj, subTotal) {
  ...
  document.querySelector("#all-discount").textContent =
    `-${checkDollarStringFormat(billObj.getAllDiscount())}`;
  document.querySelector("#final-total").textContent =
    checkDollarStringFormat(subTotal);
  document.querySelector(".toggle").onclick = (e) =>
    handledToggleValueChanged(e, billObj, subTotal);
...
function handleSplitAmountChanged(e, billObj) {
  ...
  if (e.target.value != "") {
    if (
      checkDollarStringFormat(e.target.value) != false &&
      parseFloat(e.target.value) >= 0 &&
      parseFloat(e.target.value) <= finalTotal
    ) {
      if (e.target.id == "card") {
        document.querySelector("#cash").value = checkDollarStringFormat(
          (finalTotal - parseFloat(e.target.value)).toFixed(2),
        );
      } else if (e.target.id == "cash") {
        document.querySelector("#card").value = checkDollarStringFormat(
          (finalTotal - parseFloat(e.target.value)).toFixed(2),
        );
      }
      setPaymentAmountToObj(billObj, finalTotal);
      ...
      e.target.value = checkDollarStringFormat(finalTotal);
      if (e.target.id == "card") {
      ...
      document.querySelector("#error").innerHTML =
        '<i class="fa-solid fa-circle-exclamation"></i> Invalid input. Please make sure it is in the correct format, larger than 0 and less than the total';
    }
  } else {
    document.querySelector("#error").innerHTML =
      '<i class="fa-solid fa-circle-exclamation"></i> Please make sure both amounts provided';
  }
}
...
function setChosenPayment(billObj) {
  ...
  document.querySelector(`#toggle-${billObj.getPayment().type}`).className +=
    " payment-chosen";
}

function setPaymentAmountToObj(billObj, finalTotal) {
  if (billObj.getPayment().type == "split") {
    if (document.querySelector("#card").value == "0.00")
      billObj.setPaymentAmount(finalTotal, billObj.getPayment().cardAmount);
    else
      billObj.setPaymentAmount(
        finalTotal,
...
function setPaymentAmountToInput(billObj) {
  if (billObj.getPayment().type != "split") {
    if (billObj.getPayment().type == "card") {
      document.querySelector("#card").value = checkDollarStringFormat(
        billObj.getPayment().cardAmount,
      );
      document.querySelector("#cash").value = "0.00";
    } else {
      document.querySelector("#cash").value = checkDollarStringFormat(
        billObj.getPayment().cashAmount,
      );
      document.querySelector("#card").value = "0.00";
    }
